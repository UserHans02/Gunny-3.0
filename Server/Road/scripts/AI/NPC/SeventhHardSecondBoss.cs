...
namespace GameServerScript.AI.NPC
{
    public class SeventhHardSecondBoss : ABrain
    {
        ...
        private void AttackingB()
        {
            ...
            if (target != null)
            {
                Body.MoveTo(target.X - 150, target.Y, "run", 1000, new LivingCallBack(NextAttackB));
            }
        }
        ...
        {
            Body.Direction = Game.FindlivingbyDir(Body);
            Body.MoveTo(181, Body.Y, "run", 1000, new LivingCallBack(ChangeDirection));
        }
        ...
        private void AttackingA()
        {
            Player target = Game.FindRandomPlayer();
            if (target != null)
                Body.MoveTo(target.X - 150, target.Y, "run", 1000, new LivingCallBack(NextAttackA));
        ...
        {
            Body.MoveTo(1477, Body.Y, "run", 1000, new LivingCallBack(PersonalAttack));
        }
        ...
        private void GoAttacking()
        {
            ...
            if (!IsEixt && target != null)
            {
                ...
                foreach (Player player in players)
                {
                    ...
                    {
                        player.AddEffect(new ContinueReduceBloodEffect(2, 900), 0);
                    }
                }
            }
